...
class FranchiseretailerController extends BaseController {
	...
	*/
	...
	public function getIndex()
	{
		...
		if($userdata && $userdata->status=="success")
		{
			...
			return View::make('franchise.retailer.manageview')->with('senddata',$senddata);
	...
	{
		return View::make('franchise.retailer.dashboard');
	}
	public function getAdd()
	{
		...
		return View::make('franchise.retailer.addview')->with('category',$category)->with('products',$products);
	}
	...
	public function postCreate()
	{
		$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('franchise/retailer/add')->withInput()->withErrors($validator);
					}
					else
					{
	...
	{
		return Redirect::to('franchise/retailer/add')->with('failure', $output->message);
	}
	else
	{
		return Redirect::to('franchise/retailer')->with('sucess',$output->message);
	}
	}
	else
	{
		return Redirect::to('franchise/retailer/add')->with('failure', 'sorry., Please try again later');
	}
}
		//return Redirect::to('franchise/retailer');
	}
	...
	public function postStore()
	{
		$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('franchise/retailer/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator);
					}
					else
					{
					...
					if ($validator1->fails()) 
					{
					return Redirect::to('franchise/retailer/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator1);
					}
					else
					{
		...
		{
		return Redirect::to('franchise/retailer')->with('success',$update->message);
		}
		else
		{
		return Redirect::to('franchise/retailer/show/'.Input::get('userIdPk'))->with('failure',"something went wrong");
	...
	public function getShow($id)
	{
		...
		return View::make('franchise.retailer.editview')->with('userdata',$userdata)->with('usercategory',$usercategory)->with('userproduct',$userproduct)->with('admincategory',$admincategory)->with('adminproduct',$adminproduct);
	}
	else
	{
	return Redirect::to('franchise/noservice');
