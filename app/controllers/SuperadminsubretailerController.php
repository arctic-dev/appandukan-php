...
class SuperadminsubretailerController extends BaseController {
	...
	*/
	...
	public function getIndex()
	{
		...
		return View::make('superadmin.subretailer.selectview')->with('senddata',$senddata);
	}
	...
	return View::make('superadmin.subretailer.manageview')->with('senddata',$senddata);
			
	}
	
	public function postSubr()
	{
		Session::put('admin_id',Input::get('userId'));
		return Redirect::to('superadmin/subretailer/manageview');
		
	}
	
	public function getDashboard()
	{
		return View::make('superadmin.subretailer.dashboard');
	}
	public function getAdd()
	{
		...
		return View::make('superadmin.subretailer.addview')->with('senddata',$senddata)->with('category',$category)->with('products',$products);
	}
	...
	public function postCreate()
	{
		$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('superadmin/subretailer/add')->withInput()->withErrors($validator);
					}
					else
					{
		...
		$op1=json_encode($finaloutput); //exit;
	...
	{
		return Redirect::to('superadmin/subretailer/add')->with('failure', $output->message);
	}
	else
	{
		return Redirect::to('superadmin/subretailer')->with('sucess','registerd successfully');
	}
	}
	else
	{
		return Redirect::to('superadmin/subretailer/add')->with('failure', 'sorry., Please try again later');
	}
}
		return Redirect::to('superadmin/subretailer');
	}
	...
	public function postStore()
	{
		$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('superadmin/subretailer/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator);
					}
					else
					{
					...
					if ($validator1->fails()) 
					{
					return Redirect::to('superadmin/subretailer/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator1);
					}
					else
					{
		...
		{
		return Redirect::to('superadmin/subretailer/manageview')->with('success',$update->message);
		}
		else
		{
		return Redirect::to('superadmin/subretailer/show/'.Input::get('userIdPk'))->with('failure',"something went wrong");
	...
	public function getShow($id)
	{
		...
		return View::make('superadmin.subretailer.editview')->with('userdata',$userdata)->with('usercategory',$usercategory)->with('userproduct',$userproduct)->with('admincategory',$admincategory)->with('adminproduct',$adminproduct);
	}
	else
	{
