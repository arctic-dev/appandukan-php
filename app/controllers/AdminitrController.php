...
class AdminitrController extends BaseController {
	...
	*/
	...
	public function getIndex()
	{
		...
		$itrusers=Services::itrusers($input);
		//print_r($itrusers->status); exit;
		if($itrusers && $itrusers->status=="success")
		{
		return View::make('admin.itr.manageuser')->with('itrform',$itrusers->itUploads);	
		}
		else
		{
	...
	public function getItr()
	{
		$input=json_encode(array(
			));

		$itrhistory=Services::itrhistory($input);
		//print_r($itrhistory); exit;
		if($itrhistory && $itrhistory->status=="success")
		{
		return View::make('admin.itr.manageview')->with('itrhistory',$itrhistory->itUploads);	
		}
		else
		{
			return Redirect::to('admin/noservice');
