...
class FranchisesubretailerController extends BaseController {
	...
	*/
	...
	public function getIndex()
	{
		...
		if($userdata && $userdata->status=="success")
		{
			...
			return View::make('franchise.subretailer.selectview')->with('senddata',$senddata);
	...
	return View::make('franchise.subretailer.manageview')->with('senddata',$senddata);
	...
	public function postSubr()
	{
		...
		return Redirect::to('franchise/subretailer/manageview');
	...
	{
		return View::make('franchise.subretailer.dashboard');
	}
	public function getAdd()
	{
		...
		return View::make('franchise.subretailer.addview')->with('senddata',$senddata)->with('category',$category)->with('products',$products);
	}
	...
	public function postStore()
	{
		$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('franchise/subretailer/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator);
					}
					else
					{
					...
					if ($validator1->fails()) 
					{
					return Redirect::to('franchise/subretailer/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator1);
					}
					else
					{
		...
		{
		return Redirect::to('franchise/subretailer/manageview')->with('success',$update->message);
		}
		else
		{
		return Redirect::to('franchise/subretailer/show/'.Input::get('userIdPk'))->with('failure',"something went wrong");
	...
	public function getShow($id)
	{
		...
		return View::make('franchise.subretailer.editview')->with('userdata',$userdata)->with('usercategory',$usercategory)->with('userproduct',$userproduct)->with('admincategory',$admincategory)->with('adminproduct',$adminproduct);
	}
	else
	{
	return Redirect::to('franchise/noservice');
