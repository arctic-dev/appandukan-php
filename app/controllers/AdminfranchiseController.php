...
class AdminfranchiseController extends BaseController {
	...
	*/
	...
	public function getIndex()
	{
		...
		if($userdata)
		{
			...
			return View::make('admin.franchise.manageview')->with('senddata',$senddata);
	...
	{	$categorydata=Sample::category(Session::get('user_name'));
		...
		return View::make('admin.franchise.addview')->with('category',$category)->with('products',$products);
	}
	
	public function getDashboard()
	{
		return View::make('admin.franchise.dashboard');
	}
	...
	public function postCreate()
	{
		$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('admin/franchise/add')->withInput()->withErrors($validator);
					}
					else
					{
	...
	{
		return Redirect::to('admin/franchise/add')->with('failure', $output->message);
	}
	else
	{
		return Redirect::to('admin/franchise')->with('sucess','registerd successfully');
	}
	}
	else
	{
		return Redirect::to('admin/franchise/add')->with('failure', 'sorry., Please try again later');
	}
}
...
$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('admin/franchise/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator);
					}
					else
					{
					...
					if ($validator1->fails()) 
					{
					return Redirect::to('admin/franchise/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator1);
					}
					else
					{
		...
		{
		return Redirect::to('admin/franchise')->with('success',$update->message);
		}
		else
		{
	...
	public function getShow($id)
	{
		...
		return View::make('admin.franchise.editview')->with('userdata',$userdata)->with('usercategory',$usercategory)->with('userproduct',$userproduct)->with('admincategory',$admincategory)->with('adminproduct',$adminproduct);
	}
	else
	{
	return Redirect::to('admin/noservice');
