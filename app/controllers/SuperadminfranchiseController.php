...
class SuperadminfranchiseController extends BaseController {
	...
	*/
	...
	public function getIndex()
	{
		...
		return View::make('superadmin.franchise.selectview')->with('senddata',$senddata);
	}
	...
	return View::make('superadmin.franchise.manageview')->with('senddata',$senddata);
			
	}
	
	public function postFran()
	{
		Session::put('admin_id',Input::get('userId'));
		return Redirect::to('superadmin/franchise/manageview');
	...
	public function getAdd()
	{
		...
		return View::make('superadmin.franchise.addview')->with('userdata',$userdata)->with('category',$category)->with('products',$products);
	}
	
	public function getDashboard()
	{
		return View::make('superadmin.franchise.dashboard');
	}
	...
	public function postCreate()
	{
		$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('superadmin/franchise/add')->withInput()->withErrors($validator);
					}
					else
					{
	...
	{
		return Redirect::to('superadmin/franchise/add')->with('failure', $output->message);
	}
	else
	{
		return Redirect::to('superadmin/franchise')->with('sucess','registerd successfully');
	}
	}
	else
	{
		return Redirect::to('superadmin/franchise/add')->with('failure', 'sorry., Please try again later');
	}
}
		//return Redirect::to('superadmin/franchise');
	}
	...
	public function postStore()
	{
		$postData = Input::all();
					...
					if ($validator->fails()) 
					{
					return Redirect::to('superadmin/franchise/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator);
					}
					else
					{
					...
					if ($validator1->fails()) 
					{
					return Redirect::to('superadmin/franchise/show/'.Input::get('userIdPk'))->withInput()->withErrors($validator1);
					}
					else
					{
		...
		{
		return Redirect::to('superadmin/franchise/manageview')->with('success',$update->message);
		}
		else
		{
	...
	public function getShow($id)
	{
		...
		return View::make('superadmin.franchise.editview')->with('userdata',$userdata)->with('usercategory',$usercategory)->with('userproduct',$userproduct)->with('admincategory',$admincategory)->with('adminproduct',$adminproduct);
	}
	else
	{
