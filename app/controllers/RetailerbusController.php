...
class RetailerbusController extends BaseController {
...
{	
	...
	//print_r($city);
		return View::make('retailer.bus.search')->with('city',$city);
}
else
{
return View::make('retailer/static');
	...
	//print_r(array_unique($busdetail)); exit;
		...
		return View::make('retailer.bus.searchdetail')->with('busdetail',$busdetail)->with('dateofjourney',$dateofjourney)->with('input',Input::all());
	}
...
//echo $html;
	...
	{
	//print_r(Input::all());
	return View::make('retailer.bus.bookpage')->with('input',Input::all());
	}
